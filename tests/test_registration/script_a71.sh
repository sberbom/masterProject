...
adb -s $device1 logcat SignupActivity:D SignupActivity:D \*:S >> log_device_a71.txt &
# Delete stored data
...
# Start test
rounds=100
echo "Number of rounds: $rounds" > log_device_a71.txt
for ((i=1; i<=$rounds; i++))
do
    echo "round $i of $rounds"
    echo "round $i of $rounds" >> log_device_a71.txt
    adb -s $device1 shell am start -n com.example.masterproject/com.example.masterproject.activities.MainActivity
    sleep 2
    adb -s $device1 shell input tap 50 170 #hamburger menu
    sleep 0.5
    ...
    sleep 0.5
    adb -s $device1 shell input tap 220 460 #select email
    sleep 0.5
    adb -s $device1 shell input text 'test@mail.no'
    sleep 0.5
    adb -s $device1 shell input tap 190 1000 #singup button
    sleep 13 
    adb -s $device1 shell input tap 50 170 #hamburger menu
    sleep 0.5
    adb -s $device1 shell input tap 220 880 #delete all data
